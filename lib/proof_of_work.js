...
POWControl.StartMining = async function (shardID) {
    ...
    // Was unable to obtain next block, no responses? shard complete?
    if (!block){
        return false;
    }

    POWControl.CurrentShards[shardID || "mainchain"] = block;
    ...
    for (var v of helper.orderedHashFields(block, ignoreNonce = true)) {
