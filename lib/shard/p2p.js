...
p2p.reqResponses = async function (req, res) {
    ...
    var shard = ShardLogicController.PoolManager.getResponsePool(shardID);//activeShards[shardID];
...
p2p.sendNewBlock = async function (req, res) {
    ...
    do more verification like timestamps and checking the respondents in detail
...
p2p.broadcastBlock = async function (shardID, blockObj) {
    ...
    var shard = activeShards[shardID];
    shard.peers = ["localhost:9011", "localhost:9012"];
    if (!shard.peers) {
