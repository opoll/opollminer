...
var databases = {
  ShardMiner: level( `./db/${tAutomatedInj}shard_miner` ), // General K-V Store
  Polls: level( `./db/${tAutomatedInj}polls` ), // Maps: pollHash -> /poll/poll
  Shards: level( `./db/${tAutomatedInj}shards` ), // Maps: pollHash -> /shard/shard
  ShardBlocks: level( `./db/${tAutomatedInj}shard_blocks` ), // Maps: blockHash -> /shard/block
  ResponsePool: level( `./db/${tAutomatedInj}shard_pool` ), // Maps: pollHash -> Array of /poll/response
};
...
controller.localStoreValidShard = function( shardObject, cb ) {
  databases.Shards.put( controller.pullShardIdentifier( shardObject ), JSON.stringify( shardObject ), cb );
};
...
controller.wipePollShard = function( shardObject, cb ) {
  ...
  databases.Shards.del( controller.pullShardIdentifier( shardObject ), cb );
};
