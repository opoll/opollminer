...
//Remove a working shard so it doesnt continue mining
lib.removeWorkingShard = async function(pollHash){
  workedShardcache = workedShardcache.filter(item => item != pollHash);
...
lib.persistMineShard = async function( pollHash ) {
  return new Promise( async function( resolve ) {
    ...
    hook.Call("BeginMiningShard", pollHash);
