...
// Refresh the active shards periodically. To do this we attatch the
// lib.fetchActiveShards() to the 'networkTick'
hook.Add("networkTick", "refreshActiveShards", async function(){
    await lib.fetchActiveShards();
})
...
lib.refreshResponses = async function (pollHash) {
    var activeShards = await lib.getActiveShards();
    var res = await axios.get(ShardNetworkModule.API_URL + `/polls/${pollHash}/responses`).then(function (res) {
        if (res && res.data) {
