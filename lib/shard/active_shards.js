...
lib.refreshResponses = async function (pollHash) {
    var activeShards = await lib.getActiveShards();
    var res = await axios.get(ShardNetworkModule.API_URL + `/shard/${pollHash}/responses`).then(function (resp) {
        activeShards[pollHash].responses = resp.data;
    }).catch(function (err) { });
}
...
/* convert this to p2p, ensure no fork chains*/
