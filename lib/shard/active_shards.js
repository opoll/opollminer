...
var ShardBlockHelper = var PollHelper = require('@openpoll/helpers').shardBlock;
...
lib.getLatestBlock = async function (pollHash) {
    return new Promise(function (resolve) {
        var res = axios.get(ShardNetworkModule.API_URL + `/shard/${pollHash}/${blockid}/latestblock`).then(function (resp) {
            resolve(resp.data);
        }).catch(function (err) {
            resolve({});
        });
