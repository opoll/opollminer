...
lib.fetchActiveShards = function() {
  return new Promise( function( resolve ) {
    ...
    NetworkModule.queryAPIServer( "getshards", function( shardList ) {
      ...
      lib.recordActiveShards( shardList, resolve );
    } );
  } );
}
...
*/
lib.recordActiveShards = function( pollHashArr ) {
  return new Promise( async function( resolve ) {
    // Get the latest set of shards being mined
    var activeShardsArr = await lib.getActiveShards();
    ...
    // Store
    databases.ShardMiner.put( 'active_shards', JSON.stringify( activeShardsArr ), resolve );
  } );
};
