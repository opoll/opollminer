...
hook.Add("blockValidated", "createBlockLedger", async function(block){
    ...
    for (var transHash in block.transactions){
        ...
        */
        ...
        utxo[transaction.senderAddress] = (utxo[transaction.senderAddress] || 0) - transaction.amount;
        utxo[transaction.receiverAddress] = (utxo[transaction.receiverAddress] || 0) + payment;
        
    }

    var utxo = await ledger.blockhashToUTXO(block.hash);
    await utxo.open();
    await utxo.put("data", utxo);
    await utxo.close();
});
