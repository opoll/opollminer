...
// External libraries
const helpers = require('@openpoll/helpers').generic;
...
/****************************************************/
    ...
    // Description: Persists a notification and sends it to the GUI for display. Notification types as of
    // now are "shard", "mainchain", and "general"
    // Data Emitted: notificationData
    lib.newNotification = function(notification){
        // Give the notification an ID & dateCreated
        notification['id'] = helpers.generateRandomId(16);
        notification['dateCreated'] = dateHelpers.nowAsEpoch();
        ...
        databases.Notifications.get("notifications", function(err, notifsJson){
            ...
            databases.Notifications.put("notifications", JSON.stringify(notifications), function(err){
                // Send data to the GUI once save is complete
