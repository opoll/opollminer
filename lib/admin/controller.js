...
controller.checkToken = function(req, res) {
    const tokenToCheck = (req.params && req.params.token) || req.body.token;
...
controller.getWalletsInfo = function(req, res) {
    ...
    databases.Wallets.get("wallets", function(err, walletsJson){
        if(err){
            // If there is no wallets db then set the array as empty
            walletsJson = "[]";
        }
...
// Gets all of the miner's notifications from the NotificationsDb
controller.getNotifications = function(req, res){

    // Pull the notifications from the db
    databases.Notifications.get("notifications", function(err, notifsJson){
        if(err){
            // If there is notifications db then set the array as empty
            notifsJson = "[]";
        }

        // Convert json string into notifications array
...
controller.deleteNotification = function(req, res){
    ...
    databases.Notifications.get("notifications", function(err, notifsJson){
        ...
        // Respond appropriately
        if(isFoundAndDeleted){
            // Notification was found and was deleted
            // Re-persist the updated notifications array
...
controller.deleteNotifications = function(req, res){
    const cutoffEpoch = req.query.cutoff;
    ...
    databases.Notifications.get("notifications", function(err, notifsJson){
        ...
        // Convert json string into notifications array
        let notifications = JSON.parse(notifsJson);

        /* Delete notifications with an epoch less than the cutoff */

        let notificationsDeleted = []; // Empty array to keep track of notification Id's deleted

        // Delete notifications falling behind the cutoff
        notifications.forEach( function(notification, index){
            if(notification.dateCreated < cutoffEpoch){
                // Update notificationDeleted array
                notificationsDeleted.push(notification.id);

                // Mark the notification's position as empty
                delete notifications[index];
            }
        });

        // Apply dummy filter to collapse empty positions. (Array enumeration skips missing elements)
        notifications = notifications.filter(function() { return true; });
        ...
        databases.Notifications.put("notifications", JSON.stringify(notifications), function(err){
            ...
            return res.json({
                ...
                numDeleted: notificationsDeleted.length
            });
        });
