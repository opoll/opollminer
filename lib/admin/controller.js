...
const cryptoHelpers = require('../util/cryptoHelpers');
...
controller.serveAdminInfo = function(req, res) {
    if(!adminInfoServed){
        ...
        // Generate adminAuthToken and save to leveldb
        const adminAuthToken = adminHelpers.generateAdminToken();
        databases.ShardMiner.put("adminAuthToken", adminAuthToken, function(err){
            if (err) return console.log('There was a problem saving the admin auth token.', err);
        });

        res.status(200); // Set response status
        console.log( chalk.green( "AdminAuthToken: " + adminAuthToken ) ); // Log the token to the console for miner reference
        adminInfoServed = true; // Shutdown endpoint

        // Respond with appropriate json
        return res.json({
            adminAuthToken, // same as -> "adminAuthToken": adminAuthToken -> ECMAScript 2016 notation for a variable whose key is the same as variable name that holds its value
            minerType: 'Shard'
        });
    }
}
...
/***************************************************************/
/*                 Miner Wallet Admin Endpoints                */      // TODO: Protect sensitive endpoints with double password authentication
/***************************************************************/
...
controller.getWalletsInfo = function(req, res) {
    ...
    databases.Wallets.get("wallets", function(err, walletsJson){
        ...
        walletsArray.forEach(function(wallet) {
            ...
            // Must go out onto network to get wallet's balance or do we keep it
...
controller.addWallet = function(req, res) {
    ...
    const privateKey = ""; // Library doesn't support fetching private key. Go back and solve later.
    newWallet.publicKey = publicKey;
    newWallet.privateKey = privateKey;

    // Generate & set address
    newWallet.address = cryptoHelpers.generateWalletAddress(publicKey);
