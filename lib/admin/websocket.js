...
module.exports.initialize = function(server) {
    ...
    io.on('connection', function(socket){
        ...
        // If origin is not this local machine disconnect the connection
        if(origin !== 'http://localhost:4200') {
            socket.disconnect(true);
            console.log(chalk.red("[NET] ") + "Websocket connection disconnected.");
        }

        // Origin is this local machine. Connection successful.
        console.log(chalk.magenta("[NET] ") + "Websocket connection created.");
    });
};
