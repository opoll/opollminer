...
lib.ValidateTransaction = async function (trans, peer) {
    ...
    /* temp checker until validate schema is fixed */

    if (!trans || trans.senderAddress == undefined || trans.receiverAddress == undefined || trans.timestamp == undefined || trans.signature == undefined || trans.amount == undefined || trans.hash == undefined || trans.respondentPublicKey == undefined) {
        console.log("invalid schema");
        return false;
    }
    if (!helpers.transaction.validateSchema(trans)) {
        console.log("invalid schema1");
        return false;
    }
