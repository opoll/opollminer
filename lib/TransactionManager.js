...
lib.ValidateTransaction = async function(trans, peer) {
    ...
    var valid = helpers.transaction.validateSchema(trans);
    console.log(valid);
...
lib.GetPool = async function () {
    ...
    for (var transHash in newList) {
        ...
        var used = await MainLogicController.LedgerManager.usedTransaction(latestBlock.hash, transHash);

        if (used) {
            console.log("TRANS IN USE!");
            delete newList[transHash];
        } else {
            // check if sender can afford this transaction;
            var canAfford = await MainLogicController.WalletManager.canAfford(transaction.senderAddress, transaction.amount, tempAccountLedger);
            if (!canAfford) {
                delete newList[transHash];
            } else {
                var currentAmount = await MainLogicController.LedgerManager.getAmount(latestBlock.hash, transaction.senderAddress);
...
/*
    This function used to refresh all transactions by querying peers to
    get them, however this is now deprecated. For the time being,
    transactions must be pushed to all nodes by the sender / txn author.
*/
/*
hook.Add("networkTick", "refreshTransactions", async function () {
    var pool = await lib.GetPool()
    console.log(pool);
