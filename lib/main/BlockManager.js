...
var GenesisBlock = {
    ...
    /* test ledger with baked in wallets */
    ledger: {
        wallets: {
            "0x001": 1000,
        },
...
var WalletManager = require("../WalletManager")(MainLogicController);
var TransactionManager = require("../TransactionManager")(MainLogicController);
...
// generate the next block for the main chain
lib.generateNextBlock = async function () {
    ...
    blankBlock.transactions = TransactionManager.GetPool()
...
// Validate a shard chain you recieve from a peer
lib.ValidateShard = async function (shardID, shardChain) {
    ...
    */
    ...
    for (var blockID in shardChain) {
        ...
        var validated = await BlockManagerBase.ValidateBlock();
    ...
    // Check if the last block is the actual last block;
    if (latestBlock.isLastBlock) {
        return true;
    }

    // they sent us a shard that was incomplete
    return false;
}
