...
var GenesisBlock = {
    ...
    ledger: {
        wallets: {
            "OPEN12V1bnRoASzNB9CJNVHQ89LeeUygA4ib3AkL1aPePOLL": 1000000000,
        },
        transactions:{
...
lib.BlockManagerBase = BlockManagerBase;
...
// Keep a global reference to the BlockManager DB Store
lib.db = db;

// Return list of shards not already in chain
lib.filterShards = function(blockhash){
    return new Promise(async function(resolve){
        var pool = await MainLogicController.LedgerManager.getEntireShardPool();
        var shardLedger = await MainLogicController.LedgerManager.blockhashToLedger(blockhash, "shards");
        var newList = Object.assign({}, pool);
        shardLedger.createReadStream().on('data',function(data){
            if (pool[data.key]){
                delete newList[data.key];
            }
...
// convert a shard blockhash into CompletedShard

lib.responseLedgerToJSON = function(ledger){
    return new Promise(function(resolve){
        var resps = {};
        ledger.createReadStream().on('data',function(data){
            resps[data.key] = data.value;
        }).on('end',function(){
            resolve(resps);
        })
    });
...
// Takes a block hash and determines completed shard list
lib.blockhashToCompletedShardList = async function(blockhash){
 
    var pool = await lib.filterShards(blockhash);

    var shards = await global.ShardLogicController.ActiveShardsModule.getActiveShards();

    var pollCost = 1000000000;

                   //38300000000
    var completedShards = {};
    
    // Go through unprocessed polls in the pool
    for( var pollHash in pool ) {
        /* HANDLE FEES N SHIT */
        var pollData = shards[pollHash];

        var completedBlocks = await MainLogicController.LedgerManager.getShardPool(pollHash);

        // await MainLogicController.LedgerManager.blockhashToLedger(blockhash, "responses");
        var selectedHash;
        // TODO: CALCULATE WHICH CHAIN TO PICK
        for (var blockhs in completedBlocks){
            selectedHash = blockhs;
            break;
        }

        // no blocks found so dont add this shard to block
        if (!selectedHash){
            continue;
        }

        var blockChain = await BlockManagerBase.getEntireChain(selectedHash);

        var minerBlockCounts = {};
        var rewardAddressCounts = {};

        // Loop through and count up the number of...
        // 1) Blocks mined by each miner
        // 2) Responses associated with each reward address
        for (var blockID in blockChain) {
            var block = blockChain[blockID];
            
            // Add this block to the count of blocks mined by this miner
            minerBlockCounts[block.minerAddress] = (minerBlockCounts[block.minerAddress] || 0) + 1;

            // Loop through all responses in this block
            Object.keys(block.responses).forEach((respondentAddress) => {
                var resp = block.responses[respondentAddress];
                // Figure out how many responses were submitted under this respondent address
                rewardAddressCounts[resp.rewardAddress] = (rewardAddressCounts[resp.rewardAddress] || 0) + 1;
            });
        }
        // Calculate reward units
        var rewardPerShardBlock = ((pollCost * 0.05) / Object.keys(blockChain).length);
        var rewardPerResponse = ((pollCost * 0.85) / pollData.maxRespondents);
        rewardPerShardBlock = floor(rewardPerShardBlock);
        rewardPerResponse = floor(rewardPerResponse);
        // Calculate the total rewards
        var shardRespondentRewards = {};
        var shardMinerRewards = {};
        
        // Add shard miner rewards
        Object.keys(minerBlockCounts).forEach((minerAddress) => {
            // Get the number of blocks mined by this miner
            let minerBlockCnt = minerBlockCounts[minerAddress];

            // Add their reward
            shardMinerRewards[minerAddress] = (minerBlockCnt * rewardPerShardBlock);
        });
        // Add respondent miner rewards
        Object.keys(rewardAddressCounts).forEach((rewardAddress) => {
            // Get the number of responses submitted under this address
            let rewardAddressCnt = rewardAddressCounts[rewardAddress];

            // Add their reward
            shardRespondentRewards[rewardAddress] = (rewardAddressCnt * rewardPerResponse);
        });

        // Add calculated rewards into completed shard list
        completedShards[pollHash] = {
            shardRespondentRewards: shardRespondentRewards,
            shardMinerRewards: shardMinerRewards,
        }

        //responseLedger.close();
    }
    //console.log(completedShards);
    return completedShards;
    //console.log("UNUSED:",pool)
}
...
lib.GetShardPool = async function(){
    ...
    var pool = await lib.blockhashToCompletedShardList(latestBlock.hash);
    return pool;
}
...
/*
    Returns an array of mainchain block HASHES corresponding
    to the provided block id
*/
lib.mainchainBlockIdToBlockHashArray = function( blockId ){
    return new Promise(function( resolve ) {
        // Query the Level DB block id -> blocks hash array mapping
        db.blocksByID.get( blockId, function( err, value ) {
            // If the blockchain isn't that long..
            if( err ) {
                console.log( `ERROR! No mainchain blocks associated with id: ${blockId}` );
                return resolve( false );
            }

            // Attempt to parse block JSON
            try {
                // Convert the DB JSON Strong to an object
                let mainchainBlock = JSON.parse( value );

                // Return the mainchain block
                return resolve( mainchainBlock );
            } catch (err) {
                console.log( `CRITICAL ERROR! Unable to process mainchain block, id: ${blockId}` );
                return resolve( false );
            }
        });
    });
...
/*
    This function returns the latest block on the longest blockchain within
    the mainchain. It relies heavily on `lib.getLatestID` to do the heavy lifting.
    If the longest chain is not found, it returns the genesis block.
*/
lib.getLongestChain = async function () {
    ...
    if (latest <= 0) {
        return MainLogicController.createGenesis();
    }

    console.log(`BLOCKCHAIN LENGTH ${latest}`);

    // Get a list of blocks that are all 'longest chains'
    var blocks = await lib.mainchainBlockIdToBlockHashArray(latest);
    console.log( "CALLED GET LONGEST CHAIN, BLOCKS: " );
    console.log( blocks );
...
lib.ValidateBlock = async function (block, peer) {
   ...
   var ledger = Object.assign({}, block.ledger);
    ...
    var blocks = await lib.mainchainBlockIdToBlockHashArray(blockID);
    blocks = blocks || {};
...
/*
    ...
    valid or invalid.
*/
lib.ValidateShard = async function (shardID, shardChain, peer) {
    ...
    */
    ...
    // they sent us a shard that does not exist. ignore it
    if (!shard){ return false; }

    var max = Object.keys(shardChain).length;
    for (var i=1; i<= max; i++){
        var block = shardChain[i];
        // use shard validation
        var validated = await BlockManagerBase.ValidateBlock(block, peer);
    ...
    var latestBlock = shardChain[max];

    var ledger = await MainLogicController.LedgerManager.blockhashToLedger(latestBlock.hash, "responses");
    await ledger.open();
    var totalResponses = await MainLogicController.LedgerManager.countKeys(ledger);
    await ledger.close();

    // invalid response number
    if (totalResponses != shard.maxRespondents){
        return false;
    }
