...
lib.ValidateBlock = async function( block, peer ) {
    ...
    {
        console.log(`Block invalid; missing fields: ${block.hash}`);
        return false;
    }
    ...
    if (!ShardLogicController.POWController.CheckDiff(block)) {
        console.log(`Invalid block, wrong difficulty ${block.hash}`);
        return false;
    }

    // BIG TODO: Rehash the block
    if( blockHash != block.hash ) {
        console.log(`Block contains incorrect hash ${block.hash}`);
        return false;
    }
    ...
    if (Object.keys(block.responses || {}).length <= 0) {
        console.log(`Invalid block, no responses: ${block.hash}`);
        return false;
    }

    // Make sure we have the previous block
    if (!previousBlock) {
        console.log("Invalid block, could not find previous block!");
        return false;
    }

    // Make sure the block id is >= 0
    if (previousBlock.blockId != (blockID - 1)) {
        console.log(`Invalid block, bad block id provided`);
        return false;
    }
    ...
    if (!validated) {
        console.log(`Invalid block, the previous block could not be validated: ${previousBlock.hash}`);
        return false;
    }
    ...
    **************************************/
    ...
    // Make sure the shard is active..
    if( !shard ) {
        console.log(`Invalid block, the shard is no longer active: ${pollHash}`);
        return false;
    }

    // Get the shard ledger for the previous block
    var ledger = await ShardLogicController.LedgerManager.blockhashToLedger(block.prevHash, "responses");

    // Get the number of responses from the previous block
    try {
        var currentResponseCount = await ShardLogicController.LedgerManager.countKeys(ledger);
        console.log(`Shard currently has ${currentResponseCount} responses (${pollHash})`);
    } catch(ex) {
        console.log(`CRITICAL ERROR! Cannot load prevous shard block hash ledger! Shard: ${block.prevHash}`);
        return false;
    }

    // Number of new responses
    var numNewResponses = Object.keys( block.responses );

    // Too many responses in block;
    if ( (currentResponseCount + numNewResponses) > shard.maxRespondents ){
        console.log(`Invalid block, too many responses (shard: ${pollHash})!`);
        await ledger.close();
        return false;
    }
    ...
    // Create ledger for new block
    // TODO: Refactor
    var ledger2 = await ShardLogicController.LedgerManager.blockhashToLedger(block.hash, "responses");

    // Go through all responses in the new block
    for( var respAddress in Object.keys( block.responses ) ) {
        await ledger2.put(respAddress, true);
    }

    // Close the new ledger
    await ledger2.close();

    // Add the block
    console.log(`New shard block! Id: ${block.id}, Shard: ${pollHash}`);
    await lib.AddBlock(block, peer);
    
    // We generated a block that completes the shard; call hook ShardComplete
    if( ((currentResponseCount + numNewResponses) >= shard.maxRespondents) && !peer ) {
        console.log(`Shard Completed! Shard: ${pollHash}`);
        hook.Call("ShardCompleted", pollHash);
    }
...
lib.AddBlock = async function (block, peer) {
    ...
    console.log("verified block", block.hash, block.blockId);
}
...
lib.generateNextBlock = async function ( pollHash ) {
    ...
    if( !shard ) {
        console.log( `CRITICAL ERROR: Attempt to generate block for inactive shard: ${pollHash}` );
        return false;
    }

    // Check if there are any new responses
    // TODO: Is `shard.responses` really the response pool?
    if( !shard.responses ) {
        console.log( `Cannot generate new shard block, no responses: ${pollHash}` );
        return false;
    }
   ...
   /* if (shard.maxRespondents >= Object.keys(ledger.entries).length) {
    ...
    var totalRespondents = await ShardLogicController.LedgerManager.countKeys(ledger);
    /*
    ...
    */
    ...
    for(var i=0; i<shard.responses.length; i++){
        ...
        if (!used && (totalRespondents +Object.keys(responsesCopy).length ) < shard.maxRespondents){
    ...
    newBlock.prevHash = ShardBlockHelper.hash(latestBlock);
    ...
    newBlock.hash = ShardBlockHelper.hash(newBlock);
