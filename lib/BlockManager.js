...
lib.ValidateBlock = async function (block, peer) {
    ...
    */
    ...
    var ledger = await ShardLogicController.LedgerManager.blockhashToLedger(previousBlock.hash, "responses");
    shard.maxRespondents = 45;
    await ledger.open();

    var currentResponseCount = await ShardLogicController.LedgerManager.countKeys(ledger);
    console.log("response count:",currentResponseCount + Object.keys(block.responses).length)
    // Too many responses in block;
    if ((currentResponseCount + Object.keys(block.responses).length) > shard.maxRespondents){
        console.log ("Block has more responses then requred");
        await ledger.close();
        return false;
    }
    ledger.close();
