...
lib.ValidateBlock = async function (block, peer) {
    ...
    {
        console.log("invalid block", block.hash);
        return false;
    }
   ...
   // shard.maxRespondents = 45;
    
    //await ledger.open();

    var currentResponseCount = await ShardLogicController.LedgerManager.countKeys(ledger);
    await console.log("response count:",currentResponseCount + Object.keys(block.responses).length)
    // Too many responses in block;
    if ((currentResponseCount + Object.keys(block.responses).length) > shard.maxRespondents){
        await console.log ("Block has more responses then requred");
        await ledger.close();
        return false;
    }
    await ledger.close();
    ...
    if ((currentResponseCount + Object.keys(block.responses).length) == shard.maxRespondents && !peer){
...
lib.generateNextBlock = async function ( pollHash ) {
    ...
    */
    ...
    newBlock.pollResponseArr = responsesCopy;
