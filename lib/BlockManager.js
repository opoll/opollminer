...
lib.generateNextBlock = async function (pollHash) {
    var latestBlock = lib.getLongestChain(pollHash);
    var ledger = latestBlock.ledger;

    var shard = ShardLogicController.ActiveShardsModule.getActiveShards()[pollHash];
    ...
    if (!shard.responses || Object.keys(shard.responses).length <= 0) {
        return false;
    }
    ...
    var responsesCopy = Object.assign({}, shard.responses);

    // sort through latest ledger
    for (var respAddress in ledger.entries) {
        // A response is already in a previous block so remove it the from the copied list;
        if (responsesCopy[respAddress]) {
            delete responsesCopy[respAddress];
        } else {
            // this should never be an outcome. this means a response should not be in here.
            return false;
        }
    }
